...
// Tabulation - O(c^3) time and O(c^2) space
class Solution
{
public:
    int minCost(int n, vector<int> &cuts)
    {
        ...
        cuts.insert(cuts.begin(), 0);
        sort(cuts.begin(), cuts.end());
        vector<vector<int>> dp(c + 2, vector<int>(c + 2, 0)); //+2 to avoid out of bound when accessing dp[k+1][j] and dp[i][k-1]
        for (int i = c; i >= 1; i--)
        {
            for (int j = 1; j <= c; j++)
            {
                ...
                {
                    int cut =
                        cuts[j + 1] - cuts[i - 1] + dp[k + 1][j] + dp[i][k - 1];
                    res = min(res, cut);
                }
                dp[i][j] = res;
            }

...
public:
    ...
    int m;
    ll solve(int l, int r, int dir, vector<int>& requests) {
        if(l == 0 && r == m - 1) return 0;
        if(dp[l][r][dir] != -1)  return dp[l][r][dir];
        int remaining = m - (r - l + 1); //people waiting
        int curr;
        if(dir == 0) curr = requests[l];
        else curr = requests[r];
        ll ans = LLONG_MAX;
        
  
        if(l > 0) {
            ll dist = curr - requests[l - 1];
            ans = min(ans,dist * remaining + solve(l - 1, r, 0, requests));
        }

        if(r + 1 < m) {
            ll dist = requests[r + 1] - curr;
            ans = min(ans,dist * remaining +solve(l, r + 1, 1, requests));
        }

        return dp[l][r][dir] = ans;
    }
    ...
    long long elevatorRequests(int n, int start, vector<int>& requests) {
        ...
        dp.assign(m, vector<vector<ll>>(m, vector<ll>(2, -1)));

        int startPos =
            lower_bound(requests.begin(), requests.end(), start)
            - requests.begin();

        return solve(startPos, startPos, 0, requests);
    }
};
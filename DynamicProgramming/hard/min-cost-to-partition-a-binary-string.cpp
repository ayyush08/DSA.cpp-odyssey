...
public:
    ...
    int flat;
    int enc;
    unordered_map<string, long long> dp; // start+"_"+len -> cost
    vector<long long> preSens;
    long long func(string &s, int i, int len)
    {
        string key = to_string(i) + "_" + to_string(len);
        if (dp.count(key))
            return dp[key];

        long long X = preSens[i + len - 1] - (i ? preSens[i - 1] : 0);
        long long cost = 0;
        if (X == 0)
            return dp[key] = flat;
        else
        ...
        long long splitCost = LLONG_MAX;
        if (len % 2 == 0)
            splitCost = func(s, i, len / 2) + func(s, i + len / 2, len / 2);
        return dp[key] = min(cost, splitCost);
    }

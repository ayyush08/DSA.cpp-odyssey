...
// Time Complexity: O(m*n + mlogm) where m is number of meetings and n is number of rooms
...
public:
    int mostBooked(int n, vector<vector<int>> &meetings)
    {
        ...
        vector<long long> lastAvailableAt(n,
                                          0); // ith room available at what time
        ...
        for (auto meet : meetings)
        {
            ...
            long long earlyEndingRoomTime = LLONG_MAX;
            int earlyEndingRoom = 0;
            // find first available room
            for (int room = 0; room < n; room++)
            {
                if (lastAvailableAt[room] <= start) // start hone se pehle khtm
                {
                    lastAvailableAt[room] =
                        end; // current meeting will finish at
                    roomsUsed[room]++;
                    roomFound = true;
                    break;
                }
            ...
            // if not found, store room that finishes first, pick the one that
            // will end first
            if (!roomFound)
            {
                lastAvailableAt[earlyEndingRoom] +=
                    (end - start); // duration = end-start
                roomsUsed[earlyEndingRoom]++;
            }
        }
        ...
        for (int room = 0; room < n; room++)
        {
            if (roomsUsed[room] > usage)
            {
                usage = roomsUsed[room];
                ans = room;
            }
        }
...
//  Time Complexity: O(m log n + m log m) where m is number of meetings and n is number of rooms
//  Space Complexity: O(n)

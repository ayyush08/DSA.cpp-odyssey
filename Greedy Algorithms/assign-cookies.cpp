...
public:
    int findMaximumCookieStudents(vector<int> &Student, vector<int> &Cookie)
    {
        ...
        int student_size = Student.size();
        int cookie_size = Cookie.size();

        int l = 0, r = 0;

        while (l < student_size && r < cookie_size)
        {
...
//greedy approach - start assignment from nearest cookie and when assignment is over, move to next student , At last the index we stand will be the number of students assigned with cookies

// Time Complexity: O(NlogN + MlogM + M) where N is the number of students and M is the number of cookies

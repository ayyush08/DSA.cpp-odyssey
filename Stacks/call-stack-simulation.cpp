...
public:
    vector<int> exclusiveTime(int n, vector<string> &logs)
    {
        ...
        for (auto log : logs)
        {
            ...
            {
                if (!st.empty())
                    exclusives[st.top()] += (timestamp - prev);
                st.push(id);
                prev = timestamp;
            }
            else
            {
                exclusives[st.top()] += (timestamp - prev + 1); // +1 because the end timestamp is inclusive
                st.pop(); // Pop the function that just ended
                prev = timestamp + 1; 
            }
        }
        return exclusives;
